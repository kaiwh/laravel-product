...
@php
	...
	elseif($product->image){
		$thumb = Image::resize($product->image,100,100);
	}else{
		$thumb = $placeholder;
	}
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<div class="page-header">
						<h1>@Lang('admin::product.heading.edit')</h1>
						<ul class="breadcrumb">
							...
							<li><a href="{{ route('admin.product.edit',$product->id) }}">@Lang('admin::product.heading.edit')</a></li>
						</ul>
				...
				<div class="panel-body">
					...
					@endif
					<form method="POST" action="{{ route('admin.product.edit',['id'=>$product->id]) }}" class="form-horizontal" id="form">
	            		{{ csrf_field() }}
						...
						<div class="tab-content">
				            <div class="tab-pane active" id="tab-general">
								@php
									...
									elseif($product->descriptions){
				            			foreach ($product->descriptions as $value) {
							...
							<div class="tab-pane" id="tab-data">
								...
								<div class="form-group ">
	          					...
	          					<div class="form-group">
									...
									<div class="col-sm-10">
										<input name="price" value="{{ old('price')?old('price'):$product->price }}" placeholder="@Lang('admin::product.form.price')" id="input-price" class="form-control" type="text">
									</div>
								</div>{{-- End Price --}}
								<div class="form-group">
									<label class="col-sm-2 control-label" for="input-quantity">@Lang('admin::product.form.quantity')</label>
									<div class="col-sm-10">
										<input name="quantity" value="{{ old('quantity')?old('quantity'):$product->quantity }}" placeholder="@Lang('admin::product.form.quantity')" id="input-quantity" class="form-control" type="text">
									</div>
								</div>{{-- End quantity --}}
								<div class="form-group">
									<label class="col-sm-2 control-label" for="input-minimum">@Lang('admin::product.form.minimum')</label>
									<div class="col-sm-10">
										<input name="minimum" value="{{ old('minimum')?old('minimum'):$product->minimum }}" placeholder="@Lang('admin::product.form.minimum')" id="input-minimum" class="form-control" type="text">
									</div>
								</div>{{-- End minimum --}}
								<div class="form-group ">
							...
							@php
								...
								elseif($product->toCategories){
									foreach ($product->toCategories as $value) {
								...
								@php
									...
									elseif($product->options){
										foreach ($product->options as $k=>$option) {
											$options[$k] = $option->toArray();
											$options[$k]['descriptions'] = [];
											foreach ($option->descriptions as $desc) {
												$options[$k]['descriptions'][$desc->language] = $desc->toArray();
											}
										}
									}
									$option_status = 1;
									if(!is_null(old('option_status')) && old('option_status')==0 ){
										$option_status = 0;
									}
									elseif(!$product->option_status){
										$option_status = 0;
									}
								@endphp
